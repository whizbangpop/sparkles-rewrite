...
export class Key {
    ...
    async gen_keys(
        ...
        if (ctx.guild?.ownerId !== ctx.member?.user.id) return;
        ...
        if (!ExistingKeys) {
            const KeysEmbed1 = new EmbedBuilder()
                ...
                const ActionRow1 = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(KeysButton1, KeysButton2);
    ...
    async cancel(ctx: ButtonInteraction): Promise<void> {
        await ctx.deferReply({ ephemeral: true })
    ...
    async go_ahead(ctx: ButtonInteraction): Promise<void> {
        await ctx.deferReply({ ephemeral: true })
